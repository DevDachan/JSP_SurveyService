	...
	for(int i = 0; i<list.length; i++){
		...
		if(list[i][1].equals("checkbox") && request.getParameterValues("checkbox"+list[i][0]+"[]") != null){
			for(int k = 0; k < request.getParameterValues("checkbox"+list[i][0]+"[]").length; k++){
				...
				result = surveyDAO.addHistory(sid,userID,history_index,Integer.parseInt(list[i][0]),component_num,"checkbox","2022-12-01");	
				if(result == 0){
			...
			result = surveyDAO.addHistory(sid,userID,history_index,Integer.parseInt(list[i][0]),component_num,"radio","2022-12-01");	
			if(result == 0){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('DB Error');");
			...
			result = surveyDAO.addHistory(sid,userID,history_index,Integer.parseInt(list[i][0]),1,content,"2022-12-01");	
			if(result == 0){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('DB Error');");
...
%>
<script>
		location.href = 'userSurveyResult.jsp';
</script>
