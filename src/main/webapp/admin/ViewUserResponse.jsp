...
%>
		...
		while(count < component.length && component[count].getOptionNum() == temp_id){
			if(component[count].getOptionType().equals("radio")){
				...
				if(hstep<history.length && history[hstep].getOptionNum() == component[count].getOptionNum() && history[hstep].getComponentNum() == component[count].getComponentNum()){
					buf += "<div class='option-item'><input type='radio' name='radio"+component[count].getOptionNum()+"' value='"+component[count].getComponentNum()+"' placeholder='helo' checked disabled></div>";	
					hstep++;
				}else{
				...
				buf += "<div class='option-rows'>"; 
				if(hstep<history.length && history[hstep].getOptionNum() == component[count].getOptionNum() && history[hstep].getComponentNum() == component[count].getComponentNum()){
					buf += "<div class='option-item'><input type='checkbox' name='checkbox"+component[count].getOptionNum()+"[]' value='"+component[count].getComponentNum()+"' placeholder='helo' disabled checked ></div>";
					hstep++;
				}else{
				...
				if(hstep<history.length && history[hstep].getOptionNum() == component[count].getOptionNum() && history[hstep].getComponentNum() == component[count].getComponentNum()){
					buf += "<textarea name='text" + component[count].getOptionNum()+ "' class='form-control' maxlength='2048' style='height:100px;' readonly>"+history[hstep].getContent()+"</textarea>";
					hstep++;
				}else{
					buf += "<textarea name='text" + component[count].getOptionNum()+ "' class='form-control' maxlength='2048' style='height:100px;' readonly></textarea>";
				}
				buf +="</div>";
			}
			count++;
	...
	<button type="submit" class="btn btn-primary" onClick="history.back();" style="width:100%;"> 뒤로 가기 </button>
